...
pub async fn get_config(path: String) -> Vec<ServerBlock> {
	let mut file = File::open(path.as_str()).await.expect(format!("failed to open {} !", path).as_str());
	let mut content = String::new();
	file.read_to_string(&mut content).await.expect(format!("failed to read into {} !", path).as_str());
